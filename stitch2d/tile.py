...
class MemmapTile:
    """An image tile in a mosaic. Includes necessary logic to store data as np.memmap's.
    ...
    def draw_memmap(self, others=None, path=None):
        """Creates an image from the provided tiles and stores in a memmap/bigtiff
...
class MemmapOpenCVTile(MemmapTile):
    """An image tile in a mosaic loaded and manipulated using OpenCV. Includes necessary logic to store data as np.memmap's.
    ...
    def backend_save(path, im):
        ...
        im : numpy.ndarray
            image data
        """
        print("MemmapOpenCVTile saves an uncompressed .dat file. A custom viewer is likely needed.")

...
class MemmapMosaic:
    """Stitches a mosaic from a list of tiles. Includes necessary logic to store data as np.memmap's.
    ...
    def __init__(self, path_or_tiles, tile_class=None):
        ...
        if not isinstance(path_or_tiles, str) and not isinstance(path_or_tiles[0], str):
            ...
            # Get tile class from 1D list of Tile
            if isinstance(path_or_tiles[0], MemmapTile):
                tile_class = path_or_tiles[0].__class__
            ...
            if not issubclass(tile_class, MemmapTile):
                raise ValueError(
                    f"tile_class must be Tile or subclass if inferred (got {tile_class}"
                )
        ...
        if not self.tile_class:
            self.tile_class = MemmapOpenCVTile
    ...
    def _get_tiles(self, path_or_tiles):
        ...
        for item in items:
            if item is None:
                tiles.append(self.placeholder(tiles[0]))
            elif isinstance(item, MemmapOpenCVTile):
                tiles.append(item)
            elif isinstance(item, MemmapTile):
                tiles.append(item)
            else:
                # goes here if a path was passed to the constructor
                tiles.append(self.tile_class(item))
...
class MemmapStructuredMosaic(MemmapMosaic):
    """Stitches a mosaic from a list of tiles with a known structure. Includes necessary logic to store data as np.memmap's."""
